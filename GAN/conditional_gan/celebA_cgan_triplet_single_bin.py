...
if not os.path.exists(out_dir):
    os.makedirs(out_dir)
    shutil.copyfile(sys.argv[0], out_dir + '/pretrain_uncondition_triplet_large_lr_Identity.py')
...
E = model.E_Net_conv_64(ngpu,main_gpu= gpu, inchannel=4, outchannel=1).cuda()
print(E)
...
for it in range(100000):
    if it % 10 == 0:
        ...
        c_v3 = Variable(model.set_label_celebA(c3,batch_size=mb_size)).cuda()
        X3 = Variable(X3).cuda()
        E_fake = E(torch.cat([X,c_v3],1))
        E_loss3 = criterion(E_fake,zeros_label)
        E_loss3.backward()
    ...
    if it % 500 == 0:
        print('Iter-{};E_loss: {}/{}/{}'.format(it, E_loss1.data.tolist(),
                                                E_loss2.data.tolist(),
                                                E_loss3.data.tolist()
                                          ))
