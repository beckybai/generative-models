...
# D PART ./ data_old/ d_fc / error
    ...
    fc_feature = Variable(torch.zeros([batch_size,feature_size,1,1]))
...
# Evaluator: Triplet Net
    ...
    if(epoch%check_points==0):
        ...
        g_zero_output = G(torch.cat([g_fc, zeroinput], 1))

        g_sampler2 = Variable((torch.rand([batch_size,  hidden_d,1,1]))).cuda()
        g_fc_output1 = G(torch.cat([g_fc, g_sampler2], 1))



        # owntool.save_picture(data, out_dir + "recon_epoch{}_data.png".format(epoch ),column=column)
        owntool.save_color_picture_pixel(data_g.data.tolist(), out_dir + "recon_epoch{}_data_old.png".format(epoch ),column=column,image_size=32)
