...
# D PART ./ data_old/ d_f3 / error
    ...
    d_f3 = f3.cuda()
    g_sampler = Variable((torch.rand([batch_size,hidden_d,1,1]))).cuda()
    ...
    d_real_false_decision = D(data_old/2 + d_f3_output/2)
    ...
    d_real_false_error  =    criterion(d_real_false_decision, half_label)

    d_real_error.backward()
    d_false_error.backward()
    d_real_false_error.backward()
...
# Evaluator: G
    ...
    # For evluate the variance of the generated samples
    i = np.random.randint(0,10)
    E_solver.zero_grad()
    G_solver.zero_grad()
    data_class, label = mm.batch_next(20,label=[i],shuffle=False)
    data_class = Variable(torch.from_numpy(data_class.astype('float32'))).cuda()  # Housekeeping - reset gradient
    _, _, _, f3_class = enet(data_class)  # print("heelo")
    f3_class.data.resize_(20, feature_size, 1, 1)
    f3_class = f3_class.detach()
    g_sampler = Variable(torch.rand([20, hidden_d, 1, 1])).cuda()
    G_sample_class = G(torch.cat([f3_class, g_sampler], 1))
    # E_real_value = E(data_class.detach())
    # E_gene_value = E(G_sample_class)
    # var_loss = 10*(torch.var(E_real_value) - torch.var(G_sample_class))
    # var_loss.backward()
    # G_solver.step()
    ...
    if(epoch%check_points==0):
        ...
        print("%s: D: %s/%s G: %s (Real: %s, Fake: %s) E: %s, var %s"% (epoch,
