# copy from gan_label_curve_triplet_no_label_pretrain
# copy from gan_label_curve_triplet_nl_ng 5/28
...
out_dir = './out/gan_label_curve_triplet_no_label_pretrain_{}'.format(datetime.now())
...
path = "/home/bike/2027/generative-models/GAN/conditional_gan/toys/out/gan_zero_label_curve_d/"
...
criterion_t = nn.TripletMarginLoss(p=1,margin=10)
...
def save_grad(name):
    def hook(grad):
        grads[name] = grad.cpu().data.numpy()
...
z_fixed = Variable(torch.zeros([mb_size, Z_dim])).cuda()

c_fixed_g = np.array(range(0, mode_num))
c_fixed_g = Variable(torch.from_numpy(mutil.label_num2vec(np.repeat(c_fixed_g, mb_size // (mode_num))).astype("float32")),
                   volatile=False).cuda()


c_fixed = np.zeros([mb_size,mode_num])
c_fixed = Variable(torch.from_numpy(c_fixed.astype("float32")),volatile=False).cuda()
# zc_fixed = torch.cat([z_fixed, c_fixed], 1)
mesh_c_fixed = np.zeros([grid_num*grid_num,mode_num])
mesh_c_fixed = Variable(torch.from_numpy(mesh_c_fixed.astype('float32')),volatile=False).cuda()
# zc_fixed = torch.cat([z_fixed, c_fixed],1)
...
for it in range(100000):
    ...
    D_real = D(torch.cat([X,c_fixed],1))
    D_fake = D(torch.cat([G_sample,c_fixed],1))
    ...
    if it % 1000 == 0:
        ...
        d_g_sample_cpu, G_sample = get_grad(z_fixed, 1, 'G', c= c_fixed_g,is_z=True,need_sample=True)
        G_sample_cpu = G_sample.cpu().data.numpy()
        d_mesh = (get_grad(mesh_fixed.detach(), 1, 'mesh', is_z=False,c = mesh_c_fixed)).cpu().data.numpy()
        ...
        ax.quiver(x_fixed[::3, ::3], y_fixed[::3, ::3], gd_mesh_cpu_x[::3, ::3], gd_mesh_cpu_y[::3, ::3],
                  d_mesh[::3, ::3], units='xy')
        ax.set(aspect=1, title='3 mode_Triplet_{}.png'.format(it))
        ...
        tot_pic_num = 6 # three condition: c- anti c1 - anti  c2
        row_num = 3
        fig = plt.figure()
        ...
        for i in range(mode_num):
            ...
            xc[i],cc[i] = n2v(xc[i]), n2v(mutil.label_num2vec(cc[i].astype('int')))
        ...
        for j in range(mode_num):
            # get_grad_E(gg[0][j],gg[1][j],gg[2][j],cc[j],cc[(j+1)%3],'{}_{}'.format(str(j),str((j+1)%3)))
            # get_grad_E(gg[0][j],gg[1][j],gg[2][j],cc[j],cc[(j+2)%3],'{}_{}'.format(str(j),str((j+2)%3)))
            # get_grad_E(xc[j].detach(),mesh_fixed,mesh_fixed,mc[j],mc[(j+1)%3],"mesh_{}_{}".format(str(j),str((j+1)%3)),is_z = False)
            get_grad_E(xc[j].detach(),mesh_fixed.detach(),xc[(j+1)%3].detach(),mc[j],mc[(j+1)%3],"mesh_{}_{}".format(str(j),str((j+1)%3)),is_z = False)
            get_grad_E(xc[j].detach(),mesh_fixed.detach(),xc[(j+2)%3].detach(),mc[j],mc[(j+2)%3],"mesh_{}_{}".format(str(j),str((j+2)%3)),is_z = False)
            get_grad_E(xc[j].detach(),xc_copy[j].detach(),mesh_fixed.detach(),mc[j],mc[(j+1)%3],"mesh_false_{}_{}".format(str(j),str((j+1)%3)),is_z = False) # attention!!!
        ...
        for i in range(mode_num):
            for j in range(mode_num-1): # just for cycling, no specific meaning for j.
                ...
                # plt.axis('off')
                # ax.set_aspect('equal')
                ax.set(title ='mesh_{}_{}'.format(i,(i+j+1)%3))

                gd_mesh_cpu = -grads['mesh_{}_{}'.format(i,(i+j+1)%3)]
        ...
        for i in range(mode_num):
            for j in range(mode_num-1): # just for cycling, no specific meaning for j.
                ...
                # plt.axis('off')
                # ax.set_aspect('equal')
                ax.set(title ='mesh_false_{}_{}'.format(i,(i+j+1)%3))

                gd_mesh_cpu = -grads['mesh_false_{}_{}'.format(i,(i+j+1)%3)]
                gd_mesh_cpu_x, gd_mesh_cpu_y = np.expand_dims(gd_mesh_cpu[:, 0], 1).reshape(grid_num,
                                                                                            grid_num), np.expand_dims(
                    gd_mesh_cpu[:, 1], 1).reshape(grid_num, grid_num)
                d_mesh = d_mesh.reshape(grid_num, grid_num)
