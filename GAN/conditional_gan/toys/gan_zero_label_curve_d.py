# copy from gan_zero_label_curve
# copy from gan_no_label_curve
import torch
...
out_dir = './out/gan_zero_label_curve_{}'.format(datetime.now())
...
def get_grad(input, label, name, c=False, is_z=True, need_sample = False):
    ...
    D.zero_grad()
    if (is_z):
        sample = G(torch.cat([input,c_fixed],1))
    else:
...
for it in range(200000):
    ...
    G_sample = G(torch.cat([z,c_fixed],1))
    ...
    if it % 1000 == 0:
        ...
        # mesh_fixed = torch.cat([mesh_fixed, ])

        d_mesh = (get_grad(mesh_fixed, 1, 'mesh', is_z=False,c=mesh_c_fixed)).cpu().data.numpy()
        print(d_mesh)
