...
E_solver = optim.Adam(E.parameters(), lr=lr*5)
...
for it in range(100000):
    ...
    if it % 1000 == 0:
        ...
        ax.quiver(x_fixed[::3, ::3], y_fixed[::3, ::3], gd_mesh_cpu_x[::3, ::3], gd_mesh_cpu_y[::3, ::3],
                  d_mesh[::3, ::3], units='xy')
        ax.set(aspect=1, title='3 mode_Triplet_Eloss_{}_Dloss_{}.png'.format(np.round(E_loss_t.cpu().data.numpy(),2),np.round(float(D_loss_fake.cpu().data.numpy()),2)))
