...
E_solver = optim.Adam(E.parameters(), 1e-3)
...
def get_grad(input, label, name, c=False, is_z=True):
    if (is_z):
        ...
        sample = (input) # it seems that is_z can not be False. Otherwise we cannot get the gradient.
...
for it in range(30000):
    ...
    if it % 200 == 0:
        ...
        print('Iter-{}; D_loss_real/fake: {}/{}; G_loss: {},E_loss:{}/{}'.format(it, D_loss.data.tolist(),
                                                                    D_loss.data.tolist(), G_loss.data.tolist(),
                                                                    E_loss_t.data.tolist(), E_loss_g.data.tolist()))
        X = X.cpu().data.numpy()
        G_sample = G(torch.cat([z_fixed,c_fixed],1))

        G_sample_cpu = G_sample.cpu().data.numpy()
        d_g_sample_cpu = get_grad(torch.cat([z_fixed.detach(), c_fixed], 1), 1, 'G', c=False,is_z=True)
