...
class Data_2D_Circle():
	def __init__(self, batch_size, mode_num, distance=1):
		self.mode_num = mode_num
		self.batch_size = batch_size
		self.distance = distance
		assert batch_size % (mode_num * mode_num) == 0
	...
	def batch_next(self):
		...
		for i in range(self.mode_num):
			for j in range(self.mode_num):
				random_bias_x = np.random.normal(0, 1, size=[self.mode_size, 1])
				random_bias_y = np.random.normal(0, 1, size=[self.mode_size, 1])

				mode_matrix[range((i * self.mode_num + j) * self.mode_size, (i * self.mode_num + j + 1) * self.mode_size, 1),
				0:2] = np.array(
					[pattern_matrix[i*self.mode_num+j,0] * self.distance + random_bias_x, pattern_matrix[i*self.mode_num+j,1]
					 * self.distance + random_bias_y]).transpose().reshape(
					self.mode_size, 2)
		return mode_matrix
