...
add_in_feature = 20 # Add one dimension data for the input_feature data.
...
for epoch in (range(1,num_epoches)):
    ...
    g_sampler = Variable((torch.randn([batch_size,nosie_d,hidden_d,hidden_d])+ g_f2_mean)*g_f2_var).cuda()
    # print(g_sampler.data.tolist()[0][0][0][0])
    g_f2_output = gnet(torch.cat([g_f2,g_sampler],1)).detach()
    g_f2.data.resize_(batch_size,1,24,24)
    c_v = torch.zeros([batch_size,1,28,28])
    c_v[:,1,0:24,0:24]= g_f2
    c_v = Variable(c_v.astype('float32')).cuda()
...
# ****************************************
    ...
    if(epoch%check_points==0):
        ...
        zeroinput = Variable(torch.zeros([batch_size, nosie_d, hidden_d, hidden_d])).cuda()
        g_zero_output = gnet(torch.cat([g_f2, zeroinput], 1))

        g_sampler2 = Variable((torch.randn([batch_size, nosie_d, hidden_d, hidden_d])+g_f2_mean)*g_f2_var).cuda()
        g_f2_output1 = gnet(torch.cat([g_f2, g_sampler2], 1))

        g_sampler3 = Variable((torch.randn([batch_size, nosie_d, hidden_d, hidden_d])+g_f2_mean)*g_f2_var).cuda()
