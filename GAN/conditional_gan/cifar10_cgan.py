...
for it in range(100000):
    ...
    D_fake_real = D(torch.cat([X/2+G_sample/2,c],1))

    a = np.random.rand()
    D_a_fake_real = D(torch.cat([X*a + G_sample*(1-a),c],1))
    ...
    if it % 500 == 0:
        print('Iter-{}; D_loss_real/fake: {}/{}; G_loss: {}, D_hf:{}, D_a:{}'.format(it, D_loss_real.data.tolist(),
                                                                        D_loss_fake.data.tolist(), G_loss.data.tolist(),
                                                                              D_loss_fr.data.tolist(),D_loss_a.data.tolist() ))
        c = c_label
