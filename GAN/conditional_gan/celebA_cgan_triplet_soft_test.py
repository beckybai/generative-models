...
G_model = torch.load("/home/bike/2027/generative-models/GAN/conditional_gan/cifar100_result/basic_2017-05-15 04:13:07.448488_0/G_40000.model")
D_model = torch.load("/home/bike/2027/generative-models/GAN/conditional_gan/cifar100_result/basic_2017-05-15 04:13:07.448488_0/D_40000.model")
...
for it in range(100000):
    ...
    a = np.random.rand([1])
    D_fake_real_a = D(a*X + (1-a)*G_sample)

    # D_loss_fr = criterion(D_fake_real,half_label)
    D_loss_fake = criterion(D_fake, zeros_label)
    D_loss_real = criterion(D_real, ones_label)
    if(a > 0.5):
        D_loss_a = criterion(D_fake_real_a, ones_label)

    D_loss = D_loss_real+ D_loss_fake + D_loss_a
    D_loss.backward()
    D_solver.step()
...
#
    if it % 2 == 0:
        ...
        X2, c2 = celebA.batch_next(mb_size, label= c, shuffle=False, Neigbor=True)
        X2 = Variable(X2).cuda()
        E_real = E(X2)

        X3, c3 = celebA.batch_next(mb_size, label=c, shuffle=False, Negative = True, Neigbor=True)
        X3 = Variable(X3).cuda()
        E_fake = E(X3)

        E_loss_e = criterion_t(E_anch, E_real, E_fake)
        E_loss_e.backward()
        E_solver.step()
    ...
    if it % 500 == 0:
        print('Iter-{}; D_loss_real/fake: {}/{}; G_loss: {};E_loss {}/{}'.format(it, D_loss_real.data.tolist(),
                                                                        D_loss_fake.data.tolist(), G_loss.data.tolist(),
                                                                                 E_loss_e.data.tolist(),
                                                                                 E_loss_g.data.tolist()
                                                                               ))
        print(c)
        x_g = torch.cat([z, c.float()], 1).t()
        x_g.data.resize_(mb_size, Z_dim + label_dim, 1, 1)
